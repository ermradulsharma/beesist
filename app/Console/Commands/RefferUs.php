...
class RefferUs extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'owner:reffer';
     ...
     * @var string
     */
    protected $description = 'Owner reffer notification';
    ...
    {
        $forms = PropertyManagementAgreementForm::with('user')->select('id', 'user_id', 'n_own', 'won2_email', 'fName_1', 'lName_2', 'phone_4', 'email_5', 'dob_8', 'address_10', 'status', 'created_at')->where('notify_status', '5')->whereBetween('created_at', [Carbon::now()->subDays(5), Carbon::now()->subDays(4)])->get();
        $email_data = EmailTemplate::where('title', 'REFER US')->first();
        if ($email_data->status == '1') {
            ...
            foreach ($forms as $form) {
                ...
                PropertyManagementAgreementForm::find($form->id)->update(['notify_status' => 6]);
            }
        }
    }
}
