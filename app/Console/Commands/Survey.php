...
class Survey extends Command
{
     ...
     * @var string
     */
    protected $signature = 'owner:survey';
     ...
     * @var string
     */
    protected $description = 'Owner survey notification';
    ...
    {
        $forms = PropertyManagementAgreementForm::with('user')->select('id', 'user_id', 'n_own', 'won2_email', 'fName_1', 'lName_2', 'phone_4', 'email_5', 'dob_8', 'address_10', 'status', 'created_at')->where('notify_status', '4')->whereBetween('created_at', [Carbon::now()->subDays(4), Carbon::now()->subDays(3)])->get();
        $email_data = EmailTemplate::where('title', 'SURVEY')->first();
        if ($email_data->status == '1') {
            ...
            foreach ($forms as $form) {
                ...
                PropertyManagementAgreementForm::find($form->id)->update(['notify_status' => 5]);
            }
        }
    }
}
