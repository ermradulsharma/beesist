...
class ShowingFollowup extends Command
{
     ...
     * @var string
     */
    protected $signature = 'tenant:showing_follow_up';
     ...
     * @var string
     */
    protected $description = 'TENANT SHOWING Followup';
    ...
    {
        $showings = ShowingApplication::with('user', 'property', 'agent')->select('id', 'showing_id', 'property_id', 'agent_id', 'tenant_id', 'showing_date', 'status', 'created_at')->where('notify_status', '2')->whereHas('prop_showing', function ($q) {
            $q->where('status', '"1"');
        })->whereBetween('showing_date', [Carbon::today()->format('Y/m/d H:i'), Carbon::now()->subMinutes(60)->format('Y/m/d H:i')])->get();

        $email_data = EmailTemplate::where('title', 'SHOWING FOLLOW UP')->first();
        if ($email_data->status == '1') {
            ...
            foreach ($showings as $showing) {
                ...
                ShowingApplication::find($showing->id)->update(['notify_status' => 3]);
            }
        }
    }
}
