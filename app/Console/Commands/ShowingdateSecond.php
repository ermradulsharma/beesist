...
class ShowingdateSecond extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'tenant:showing_date_second';
     ...
     * @var string
     */
    protected $description = 'TENANT BEFORE 2 HOURS OF SHOWING';
    ...
    {
        $showings = ShowingApplication::with('user', 'property')->select('id', 'showing_id', 'property_id', 'agent_id', 'tenant_id', 'showing_date', 'status', 'created_at')->where('notify_status', '"1"')->whereHas('prop_showing', function ($q) {
            $q->where('status', '"1"');
        })->whereDate('showing_date', Carbon::now()->format('Y/m/d'))->whereBetween('showing_date', [Carbon::now()->addMinutes(119)->format('Y/m/d H:i'), Carbon::now()->addMinutes(126)->format('Y/m/d H:i')])->get();

        $email_data = EmailTemplate::where('title', 'TENANT BEFORE 2H OF SHOWING')->first();
        if ($email_data->status == '1') {
            ...
            foreach ($showings as $showing) {
                if (in_array('For Rent', explode(',', $showing->property->prop_status))) {
                    ...
                    ShowingApplication::find($showing->id)->update(['notify_status' => 2]);
                } else {
