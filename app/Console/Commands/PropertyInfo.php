...
class PropertyInfo extends Command
{
     ...
     * @var string
     */
    protected $signature = 'owner:propertyinfo';
     ...
     * @var string
     */
    protected $description = 'Owner propertyinfo notification';
    ...
    {
        $forms = PropertyManagementAgreementForm::with('user')->select('id', 'user_id', 'n_own', 'won2_email', 'fName_1', 'lName_2', 'phone_4', 'email_5', 'dob_8', 'address_10', 'status', 'created_at')->where('notify_status', '0')->whereBetween('created_at', [Carbon::now()->subMinutes(10), Carbon::now()->subMinutes(5)])->get();
        $email_data = EmailTemplate::where('title', 'PROPERTY INFO')->first();
        if ($email_data->status == '1') {
            ...
            foreach ($forms as $form) {
                ...
                PropertyManagementAgreementForm::find($form->id)->update(['notify_status' => 1]);
            }
        }
    }
}
