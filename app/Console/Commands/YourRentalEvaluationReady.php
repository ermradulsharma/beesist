    ...
    public function handle()
    {
        ...
        if ($email_data && $email_data->status == '1') {
            foreach ($RentalEvaluations as $RentalEvaluation) {
                $first_name = extract_name($RentalEvaluation->name)['first_name'];
                ...
                Notification::route('email', $RentalEvaluation->email)->notify(new RentalEvaluationNotification($content, $subject));
