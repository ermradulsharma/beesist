...
trait UserAttribute
{
    public function setPasswordAttribute($password): void
    {
        ...
        $this->attributes['password'] = (strlen($password) === 60 && preg_match('/^\$2y\$/', $password)) || (strlen($password) === 95 && preg_match('/^\$argon2i\$/', $password)) ? $password : Hash::make($password);
    }
    ...
    public function getRolesLabelAttribute()
    {
        ...
        return collect($this->getRoleNames())->each(function ($role) {
            return ucwords($role);
        })->implode('<br/>');
    }
}
