...
Route::group(['as' => 'user.', 'middleware' => ['auth', 'password.expires', config('boilerplate.access.middleware.verified')]], function () {
    Route::get('dashboard', [DashboardController::class, 'index'])->middleware('is_user')->name('dashboard')->breadcrumbs(function (Trail $trail) {
    ...
    Route::get('subscribe/{id}', [SubscriptionController::class, 'subscriber'])->name('subscribers');
    Route::get('payment/success/{subscription?}', [SubscriptionController::class, 'paymentSuccess'])->name('payment.success');
});
