...
Route::group(['as' => 'auth.'], function () {
    Route::group(['middleware' => 'auth'], function () {
        ...
        Route::group(['middleware' => 'password.expires'], function () {
            ...
            // These routes require the user's email to be verified
            Route::group(['middleware' => config('boilerplate.access.middleware.verified')], function () {
                ...
                Route::group(['prefix' => 'account/2fa', 'as' => 'account.2fa.'], function () {
                    Route::group(['middleware' => '2fa:disabled'], function () {
                        Route::get('enable', [TwoFactorAuthenticationController::class, 'create'])->name('create')->breadcrumbs(function (Trail $trail) {
                            $trail->parent('frontend.user.account')->push(__('Enable Two Factor Authentication'), route('frontend.auth.account.2fa.create'));
                        });
                    });
                    Route::group(['middleware' => '2fa:enabled'], function () {
                        Route::get('recovery', [TwoFactorAuthenticationController::class, 'show'])->name('show')->breadcrumbs(function (Trail $trail) {
                            $trail->parent('frontend.user.account')->push(__('Two Factor Recovery Codes'), route('frontend.auth.account.2fa.show'));
                        });
                        Route::patch('recovery/generate', [TwoFactorAuthenticationController::class, 'update'])->name('update');
                        Route::get('disable', [DisableTwoFactorAuthenticationController::class, 'show'])->name('disable')->breadcrumbs(function (Trail $trail) {
                            $trail->parent('frontend.user.account')->push(__('Disable Two Factor Authentication'), route('frontend.auth.account.2fa.disable'));
                        });
                        Route::delete('disable', [DisableTwoFactorAuthenticationController::class, 'destroy'])->name('destroy');
                    });
                });
            });
        });
    ...
    Route::group(['middleware' => 'guest'], function () {
        ...
        // Rental Routes
        Route::post('rental-login', [LoginController::class, 'rentalApplicationLogin'])->name('rentalApplicationLogin');
        Route::post('rental-register', [RegisterController::class, 'rentalApplicationRegister'])->name('rentalApplicationRegister');

        Route::post('subscription-register', [RegisterController::class, 'subscriptionRegistration'])->name('subscriptionRegistration');
        Route::post('verify-email', [RegisterController::class, 'verifyEmail'])->name('verifyEmail');
    });
});
