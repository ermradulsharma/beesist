                ...
                <div class="card-body">
                    <div class="form-group">
                        <div class="form-check  md-form-inline-check">
                            <input class="form-check-input filled-in" name="featured" value="1" id="featured" type="checkbox" {{ $property->featured == '1' ? 'checked' : ''}}>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2" name="building_id" id="building_id">
                            ...
                            {{-- {{ $building }} --}}
                                <option value="{{ $building->id }}" {{ (optional($property)->building_id == $building->id) ? 'selected' : '' }}>{{ $building->title }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="utility_item">
                        <div class="form-check md-form-inline-check">
                            <input class="form-check-input filled-in" name="utilities[{{ $key }}]" value="1"
                                id="utilities_{{ $key }}" type="checkbox" @if (old("utilities.$key",
                                @$utilities[$key])=='1' ) checked @endif>
                        ...
                        <div class="md-form md-form-inline" id="{{ $key }}_details_box" style="margin-bottom:10px">
                            ...
                            <div class="md-form md-form-inline">
                                ...
                                <input type="text" id="utilities_{{ $key }}_num" name="utilities[{{ $key }}_num]"
                                    value="{{ old(" utilities.{$key}_num", @$utilities["{$key}_num"] ?? '' ) }}"
                                    class="form-control" placeholder=''>
                            </div>
                            <div class="md-form md-form-inline">
                                <label for="utilities_{{ $key }}_stall">Stall #</label>
                                <input type="text" id="utilities_{{ $key }}_stall" name="utilities[{{ $key }}_stall]"
                                    value="{{ old(" utilities.{$key}_stall", @$utilities["{$key}_stall"] ?? '' ) }}"
                                    class="form-control" placeholder=''>
                            </div>
                            @elseif ($key == 'storage')
                            <div class="md-form md-form-inline">
                                <label for="utilities_{{ $key }}_desc">{{ $label }} Description</label>
                                <input type="text" id="utilities_{{ $key }}_desc" name="utilities[{{ $key }}_desc]"
                                    value="{{ old(" utilities.{$key}_desc", @$utilities["{$key}_desc"] ?? '' ) }}"
                                    class="form-control" placeholder=''>
                            </div>
                            @elseif ($key == 'others')
                    ...
                    <div
                        class="form-group required form-control-wrap {{ $errors->has('pet_policy') ? 'has-error' : '' }}">
                    ...
                    @if (Auth::check() && Auth::user()->hasAnyRole('Administrator', 'Property Owner', 'Property Manager'))
                        <div class="form-group">
                            ...
                            <select class="form-control select_agents select2" name="prop_agents[]" multiple="multiple">
                                @foreach (getUsersByRole('Agent') as $agent)
                    ...
                    @elseif(Auth::check() && Auth::user()->hasAnyRole('Agent'))
                        <input type="hidden" name="prop_agents[]" value="{{ auth()->user()->id }}">
                    @else
                        <div class="form-group">
                            <label>Property Agent</label>
                            <ul style="padding: 0;">
                                @foreach (explode(',', $property->prop_agents) as $agentId)
                                    @if ($agentId)
                                        <li class="btn btn-default">{{ getUserById($agentId)->name }}</li>
                                    @endif
                                @endforeach
                            </ul>
                        </div>
                    @endif
                    @if (Auth::check() && Auth::user()->hasAnyRole('Administrator', 'Agent', 'Property Manager'))
                        <div class="form-group required form-control-wrap {{ $errors->has('user_id') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="user_id">
                                ...
                                @foreach (getUsersByRole('Property Owner') as $user)
