...
@section('title', __('Update ' . (Auth::user()->hasAllAccess() ? 'User' : 'Agent')))
@section('content')
    @php
        $routeName = $logged_in_user->hasAllAccess() ? 'user' : 'agent';
    @endphp
    <x-forms.patch :action="route(rolebased().'.auth.'. $routeName .'.update', $user)" enctype="multipart/form-data">
        <x-backend.card>
            <x-slot name="header">{{ __('Update') }} {{ $logged_in_user->hasAllAccess() ? 'User' : 'Agent' }}</x-slot>
            <x-slot name="headerActions">
                <x-utils.link class="card-header-action" :href="route(rolebased().'.auth.'. $routeName .'.index')" :text="__('Cancel')" />
            </x-slot>
            ...
            <x-slot name="body">
                <div x-data="{userType : '{{ $user->type }}'}">
                    ...
                    @if (!($user->isMasterAdmin() || $user->hasManagerAccess()))
