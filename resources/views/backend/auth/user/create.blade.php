...
@section('title', __('Create ' . (Auth::user()->hasAllAccess() ? 'User' : 'Agent')))
@php
    $routeName = Auth::user()->hasAllAccess() ? 'user' : 'agent';
@endphp
@section('content')
    <x-forms.post :action="route(rolebased().'.auth.'. $routeName .'.store')">
        <x-backend.card>
            <x-slot name="header">{{ __('Create') }} {{ Auth::user()->hasAllAccess() ? 'User' : 'Agent' }}</x-slot>
